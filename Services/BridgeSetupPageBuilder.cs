...
public static class BridgeSetupPageBuilder
{
    public static string Build(bool createdNew, string deviceId, string bridgeToken, string settingsPath)
    {
        ...
        html.AppendLine("    <p>Bridge روی این سیستم فعال است. توکن زیر را در صفحه تنظیمات VOIP CRM کپی کنید.</p>");
        ...
        html.AppendLine($"        <input id=\"bridgeToken\" readonly value=\"{token}\" />");
        ...
        html.AppendLine("    <p>بعد از کپی توکن، به CRM برگردید، «بررسی Bridge» را بزنید و سپس «اتصال این دستگاه» را انجام دهید.</p>");
        html.AppendLine("  </div>");
